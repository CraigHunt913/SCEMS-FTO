...
// ---------------------------------------------------------------- //
const v = verify(path.join(__dirname, 'fixtures', 'Code-before-refile.gs'), built);
...
const gone = everyLineSurvived(ORIGINAL, built);
ok(gone.length === 0,
...
ok(!/PARTS 2-5 are pasted at the very BOTTOM/.test(built),
...
ok(/ADD-ON : Record a skill I witnessed/.test(built),
