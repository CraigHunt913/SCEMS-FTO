...
All eighteen exit non-zero on failure. 1,223 assertions total.
...
| `portal-roster.test.js` | `rosterEmailPlanV1_`, `applyRosterEmails`, `undoRosterEmails`, `rosterConfirmCodeV1_` | an address landing on the wrong row because the roster is sorted and the list is not, an existing address being overwritten, a name that resolves to two rows being guessed at, and a cell that stopped being empty between the plan and the write |
