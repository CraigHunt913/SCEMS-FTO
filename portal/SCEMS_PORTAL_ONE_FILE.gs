...
var PORTAL_PAGE_HTML = [
  ...
  ".h{display:block;font-weight:700;font-size:1rem;line-height:1.3;}\n",
  ".m{display:block;font-size:.86rem;color:var(--ink-3);line-height:1.35;margin-top:2px;}\n",
  ...
  "    }\n",
  "    if (building.length){\n",
  "      h += '<button class=\"more\" onclick=\"toggleShow(\\'skills\\')\">'+\n",
  "           (shown('skills') ? 'Hide' : 'Show')+' the '+building.length+' still building</b",
  "utton>';\n",
  "      if (shown('skills')){\n",
  "        h += '<div class=\"panel\">';\n",
  "        building.forEach(function(s){\n",
  "          h += kv(s.skill, '<span class=\"chip c-mute\">'+esc(s.successful)+' reps</span>');",
  "\n",
  "        });\n",
  "        h += '</div>';\n",
  "      }\n",
  ...
  "  });\n",
  "\n",
  "  var h = '<h1>'+(q.length ? q.length+(q.length===1?' decision':' decisions')+' waiting'\n",
  "                           : 'Nothing waiting on you')+'</h1>';\n",
  "  h += '<p class=\"sub\">'+d.activeCount+' active '+(d.activeCount===1?'trainee':'trainees')",
  "+\n",
  "       ' &middot; '+(flagged.length ? flagged.length+' needing a look' : 'none needing a l",
  "ook')+\n",
  "       '</p>';\n",
  "\n",
  ...
  "\n",
  "  if (flagged.length){\n",
  "    h += '<h2>Needs a look</h2>';\n",
  "    flagged.forEach(function(p){ h += personCard(p.t, p.i, missingBy[p.t.name]); });\n",
  "  }\n",
  "  if (quiet.length){\n",
  "    h += '<button class=\"more\" onclick=\"toggleShow(\\'quiet\\')\">'+\n",
  "         (shown('quiet') ? 'Hide' : 'Show')+' '+quiet.length+\n",
  "         (quiet.length===1?' trainee with':' trainees with')+' nothing outstanding</button",
  ">';\n",
  "    if (shown('quiet')) quiet.forEach(function(p){ h += personCard(p.t, p.i, ''); });\n",
  "  }\n",
  "\n",
  "  if (d.forms && d.forms.length) h += '<h2>Forms</h2>'+formCards(d.forms);\n",
  "\n",
  "  h += housekeeping(d);\n",
  "\n",
  ...
  "      '<span class=\"go\">&rsaquo;</span></button>';\n",
  "  }\n",
  "  return '<div class=\"card\"><div class=\"h\">'+esc(q.skill)+'</div>'+\n",
  ...
  "  var dot = why ? (hard ? 'due' : 'soon') : 'ok';\n",
  "  return '<button class=\"card act\" onclick=\"openPerson('+i+')\"><span class=\"dot '+dot+'\"><",
  "/span>'+\n",
  "    '<span class=\"bd\"><span class=\"h\">'+esc(t.name)+' &nbsp;'+lvlChip(t.levelKey,t.level)+",
  "'</span>'+\n",
  "    '<span class=\"m\">'+esc(t.phase||'no phase set')+' &middot; '+esc(short(t.fto)||'no tra",
  "ining officer')+'</span>'+\n",
  "    (why ? '<span class=\"m\" style=\"color:var(--'+(hard?'stop':'warn')+')\">'+esc(cap(why))+",
  "'</span>' : '')+\n",
  "    '</span><span class=\"go\">&rsaquo;</span></button>';\n",
  "}\n",
  ...
  "function short(s){ s = String(s==null?'':s).trim(); return s.length > 44 ? s.slice(0,44)+'",
  "…' : s; }\n",
  "\n",
  ...
  "  var h = '<h2>Housekeeping</h2>';\n",
  "  if (subs.length){\n",
  "    h += '<button class=\"more\" onclick=\"toggleShow(\\'sameday\\')\">'+\n",
  "         (shown('sameday') ? 'Hide' : 'Show')+' '+subs.length+' same-day '+\n",
  "         (subs.length===1?'submission':'submissions')+' to settle</button>';\n",
  "    if (shown('sameday')){\n",
  "      h += '<div class=\"note n-info\"><b>Both are kept</b>Nothing has been removed. These a",
  "re the '+\n",
  "           'places where two submissions of the same kind landed on the same day and someb",
  "ody '+\n",
  "           'has to say which one stands. Open the record to read them side by side.</div>'",
  ";\n",
  "      subs.forEach(function(x){\n",
  "        h += '<button class=\"card act\" onclick=\"openRecord('+jsStr(x.trainee)+')\">'+\n",
  "          '<span class=\"dot soon\"></span><span class=\"bd\">'+\n",
  "          '<span class=\"h\">'+esc(x.trainee)+'</span>'+\n",
  "          '<span class=\"m\">'+esc(x.source)+(x.group?' &middot; '+esc(x.group):'')+\n",
  "          ' &middot; '+esc(x.when)+'</span>'+\n",
  "          '<span class=\"m\">'+x.count+' submissions on the one day</span>'+\n",
  "          '</span><span class=\"go\">&rsaquo;</span></button>';\n",
  "      });\n",
  "    }\n",
  "  }\n",
  ...
  "}\n",
  "\n",
  "function toggleShow(k){ S.show = S.show || {}; S.show[k] = !S.show[k]; render(); }\n",
  "function shown(k){ return !!(S.show||{})[k]; }\n",
